...
Captures both light-mode screenshots and a dark-mode dashboard for the README.
"""
...
def new_context(p, theme):
    ...
    context.add_init_script(
        f"try {{ window.localStorage.setItem('msp-theme', '{theme}'); }} catch (e) {{}}"
    )
...
def capture_dark(page):
    ...
    print("  captured dashboard-dark")

    # Tickets workbench (dark) - navigate to first ticket detail
    page.goto(f"{BASE_URL}/tickets", wait_until="networkidle")
    wait_for_styled(page)
    first_ticket = page.locator('main ul li a[href^="/tickets/"]').first
    ticket_href = first_ticket.get_attribute("href") if first_ticket.count() > 0 else None
    if ticket_href and ticket_href != "/tickets/new":
        ...
        wait_for_styled(page)
    page.screenshot(
        path=os.path.join(SCREENSHOTS_DIR, "tickets-workbench-dark.png"),
        full_page=False,
    )
    print("  captured tickets-workbench-dark")

    # Approvals center (dark)
    page.goto(f"{BASE_URL}/approvals", wait_until="networkidle")
    wait_for_styled(page)
    page.screenshot(
        path=os.path.join(SCREENSHOTS_DIR, "approvals-center-dark.png"),
        full_page=False,
    )
    print("  captured approvals-center-dark")
...
def main():
    ...
    with sync_playwright() as p:
        browser, context = new_context(p, "light")
        capture_light(context.new_page())
        browser.close()

        browser, context = new_context(p, "dark")
        capture_dark(context.new_page())
        browser.close()
    print("done")
